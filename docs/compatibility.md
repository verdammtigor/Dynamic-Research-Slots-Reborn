...
- **`dr_mod_version`** (Variable) - Contains the mod version (currently 1.2), enables version checks. The version is defined in `common/scripted_effects/00_dr_mod_metadata.txt` and should match the version in `descriptor.mod`.
